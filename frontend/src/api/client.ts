...
export const api = {
  ...
  updateDiscordConfig: (id: string, token: string, status: string, cover_url?: string) => request<boolean>(`/users/${id}/discord-config`, { method: 'PUT', body: JSON.stringify({ token, status, cover_url }) }),
